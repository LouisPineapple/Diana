...
static void RemoteControlSet()
{
    ...
    // 摩擦轮控制,拨轮向上打为负,向下为正
    if (rc_data[TEMP].rc.dial < 10) // 向上超过100,打开摩擦轮//原始：<-100，小于0为了使摩擦轮一直旋转。
    ...
    }else if (switch_is_down(rc_data[TEMP].rc.switch_right))
    {
        ...
        // shoot_cmd_send.load_mode = LOAD_BURSTFIRE;//修改模式
    else
        shoot_cmd_send.load_mode = LOAD_STOP;
    // 射频控制,固定每秒1发,后续可以根据左侧拨轮的值大小切换射频,
    shoot_cmd_send.shoot_rate = 6;//8
}
